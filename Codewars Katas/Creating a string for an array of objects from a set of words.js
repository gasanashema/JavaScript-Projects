...
function wordsToObject(input) {
    ...
    for (let i = 0; i<strToArr.length; i++){
        const objArr = [['name',strToArr[i]],['id',strToArr[i+1]]];
        arrOfObj.push(Object.fromEntries(objArr));
        i++;
    }
    return arrOfObj;
}
...
//approach that returns the array as a string
function wordsToObject(input) {
  let strToArr = input.split(" ");
  let result = "[";

  for (let i = 0; i < strToArr.length; i += 2) {
    result += `{name : '${strToArr[i]}', id : '${strToArr[i + 1]}'}${
      i + 2 < strToArr.length ? ", " : ""
    }`;
  }
